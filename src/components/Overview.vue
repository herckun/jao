<template>
  <div class="w-fit mx-auto flex flex-col gap-1 place-content-center place-items-center">
    <Connect />
    ...
    <div v-else>
      <div v-if="success">
        <div
          ...
          <div class="text-2xl p-1 gap-1 flex flex-col">
            <span class="text-zinc-200 font-black" :class="{ 'blur-sm': $censored }">
              ~{{
      new Intl.NumberFormat("en-EN", {
        ...
        currency: "USD",
            ...
            <span class="text-xs font-semibold" :class="{ 'text-red-400': pnl < 0, 'glowing-text': pnl > 0 }">
      ...
      new Intl.NumberFormat("en-EN", {
        ...
        currency: "USD",
          ...
          <div class="grid grid-cols-2 gap-1 w-full place-items-end md:w-fit h-fit">
            <button class="btn btn-ghost bg-white/10 col-span-1 w-full p-4" @click="refreshData">
            ...
            <button class="btn btn-ghost bg-white/10 col-span-1 w-full p-4" @click="switchCensor">
              <svg v-if="$censored" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor"
                class="w-4 h-4">
            ...
            <div role="tablist" class="tabs bg-white/10 tabs-boxed tabs-xs col-span-full w-full">
              <a @click="changeTimeFrame('hour')" role="tab" class="tab"
                :class="{ 'tab-active': $timeframe == 'hour' }">1h</a>
              <a @click="changeTimeFrame('day')" class="tab" :class="{ 'tab-active': $timeframe == 'day' }">1d</a>
              <a @click="changeTimeFrame('week')" class="tab" :class="{ 'tab-active': $timeframe == 'week' }">1w</a>
              <a @click="changeTimeFrame('month')" class="tab" :class="{ 'tab-active': $timeframe == 'month' }">1m</a>
              <a @click="changeTimeFrame('year')" class="tab" :class="{ 'tab-active': $timeframe == 'year' }">1y</a>
            </div>
          </div>
        </div>
        <div class="bg-zinc-800 rounded-xl p-1 m-2 grid grid-cols-1 md:grid-cols-2 gap-0.5 w-[95vw] md:w-[55vw]">
          <Asset v-for="(item, index) in data.result" v-show="index < counters.show" :data="item"
            :refreshing="refreshing" :fill="index == counters.total - 1 && counters.total % 2 != 0" />
          <button v-if="counters.total > 6" class="btn btn-xs btn-ghost w-fit" @click="switchShow">
